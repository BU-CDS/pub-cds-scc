...
# next deploy. Also covers this repo's zero-JS / "updated" stamp guards
# (a <script>-bearing page refused, an unstamped page refused, a clean page
# stages without pushing under DEPLOY_PUSH=0). Run directly; exits nonzero on
# any failure.
set -uo pipefail
...
mkrepo(){ # $1: dir — toy repo holding a clean page and a copy of deploy.sh
  ...
  printf '<html><body>updated monthly &middot; 2026-08-31</body></html>\n' > "$1/index.html"
...
# guard: a page with a <script> tag is refused
T5="$WORK/t5"; mkrepo "$T5"
printf '<html><script>alert(1)</script>updated monthly &middot; 2026-08-31</html>\n' > "$T5/index.html"
( cd "$T5" && DEPLOY_PUSH=0 ./deploy.sh ) >/dev/null 2>&1 \
  && bad "scripted page was accepted" || ok "scripted page refused"
# guard: a page missing the "updated monthly ·" stamp is refused
